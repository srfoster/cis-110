...
class CompiledContentService {
  ...
  /**
   * Initialize the service with compiled content (must be called by consumer app)
   * @param {Object} compiledData - The compiled content from the consumer's compiled directory
   * @param {Object} compiledData.compiledFiles - Map of file paths to compiled content
   * @param {Object} compiledData.stats - Compilation statistics
   */
  initialize(compiledData) {
    if (!compiledData || !compiledData.compiledFiles) {
      console.warn('⚠️ CompiledContentService: No compiled content provided, will use dynamic fetching only');
      return;
    }
    this.compiledFiles = compiledData.compiledFiles;
    this.stats = compiledData.stats;
    console.log('📚 CompiledContentService initialized with:', this.stats);
  ...
  async getCompiledText(path) {
    if (!this.compiledFiles) {
      throw new Error('CompiledContentService not initialized. Call initialize() with compiled content first.');
    }
    ...
    const compiled = this.compiledFiles[fileKey];
    ...
    try {
      ...
      try {
        if (this.isDevelopment) {
          ...
          const cleanPath = this.cleanPath(path);
          const fileKey = this.pathToKey(cleanPath);
          const compiled = this.compiledFiles ? this.compiledFiles[fileKey] : null;
  ...
  listFiles(type = null) {
    if (!this.compiledFiles) {
      console.warn('CompiledContentService not initialized');
      return [];
    }
    
    const files = Object.keys(this.compiledFiles);
    ...
    return files.filter(path => {
      const compiled = this.compiledFiles[path];
      return compiled && compiled.type === type;
    });
  }
   ...
   */
  getStats() {
    return this.stats || { yamlCount: 0, markdownCount: 0, totalCount: 0 };
  }
  ...
  hasFile(path) {
    try {
      const cleanPath = this.cleanPath(path);
      const fileKey = this.pathToKey(cleanPath);
      return this.compiledFiles && this.compiledFiles.hasOwnProperty(fileKey);
    } catch {
      return false;
    }
  }
